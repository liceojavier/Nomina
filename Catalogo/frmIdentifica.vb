...
'*************************************************************************************************
'*************************************************************************************************
'**  FORMULARIO:    FRMIDENTICA.VB MIEMBRO DE NOMINA.SLN                                        **
'**  AUTOR:         IVAN TRUJILLO                                                               **
'**  FECHA:         01/01/2008                                                                  **
'**  FECHA DE MOD:  --/--/----                                                                  **
'**  DESCRIPCION:                                                                               **
...
Public Class frmIdentifica
    ...
    Private Sub btnLimpiar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLimpiar.Click
        ...
        cadena = "SELECT tipoiden, nombre, usareg " &
                 "FROM tiidentifica order by tipoiden "
        ...
        End If
        textnum.Text = BuscaEscalar("select coalesce(max(tipoiden), 0) from tiidentifica ") + 1
        EnabilizarMenu(True)
    End Sub
    ...
    Private Sub Vista(ByVal dgVista As DataGridView)
        With dgVista
            ...
            .Columns(1).Width = 250
            .Columns(2).HeaderText = "Usa registro"
            .Columns(2).Width = 131
    ...
    Private Sub btnAgregar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAgregar.Click
        lpara.Clear()
        lpara("tipoiden") = textnum.Text
        lpara("nombre") = TextNombre.Text
        lpara("usareg") = cmbUsa.Text
        If validetError(textnum, ep1) = False Or validetError(TextNombre, ep1) = False Then
            Exit Sub
        End If
        Select Case accion
            Case 0
                cadena = "insert tiidentifica (tipoiden,nombre, usareg) values(@tipoiden,@nombre,@usareg)"
            Case 1

                cadena = "update tiidentifica set nombre=@nombre, usareg=@usareg where tipoiden=@tipoiden"
        End Select
    ...
    Private Sub MnuElimina_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ctxEliminar.Click
        ...
        If dgDatos.SelectedRows.Count > 0 Then
            ...
            lpara("tipoiden") = filaTemp.Item(0)
            If MsgBox("ESTA SEGURO DE ELIMINAR EL REGISTRO MARCADO", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Mensaje del Sistema") = MsgBoxResult.Yes Then
                If BuscaEscalar("select count(*) from emplegen where tipoiden=@tipoiden", ListaParametros(lpara)) > 0 Then
                    MsgBox("REGISTRO POSEE REFERENCIA", MsgBoxStyle.Critical, "Mensaje del Sistema")
                    Exit Sub
                End If
                'verificar si tiene referencia en inscripciones
                cadena = "delete from tiidentifica where tipoiden=@tipoiden"
                EjecutarQuery(cadena, ListaParametros(lpara))
