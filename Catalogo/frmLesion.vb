...
'*************************************************************************************************
'*************************************************************************************************
'**  FORMULARIO:    FRMLESION.VB MIEMBRO DE NOMINA.SLN                                          **
'**  AUTOR:         IVAN TRUJILLO                                                               **
'**  FECHA:         01/01/2008                                                                  **
'**  FECHA DE MOD:  --/--/----                                                                  **
'**  DESCRIPCION:                                                                               **
...
Public Class frmLesion
    ...
    Private Sub btnLimpiar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLimpiar.Click
        ...
        cadena = "SELECT grado, nombre " &
                 "FROM GRADOSLESION order by grado "
        ...
        End If
        textnum.Text = BuscaEscalar("select coalesce(max(grado), 0) from GRADOSLESION ") + 1
        EnabilizarMenu(True)
    End Sub
    ...
    Private Sub btnAgregar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAgregar.Click
        ...
        If validetError(textnum, ep1) = False Or validetError(TextNombre, ep1) = False Then
            Exit Sub
        End If
        lpara("grado") = textnum.Text
        lpara("nombre") = TextNombre.Text
        Select Case accion
            Case 0
                cadena = "insert GRADOSLESION (grado, nombre) values(@grado,@nombre)"
            Case 1

                cadena = "update GRADOSLESION set nombre=@nombre where grado=@grado"
        End Select
    ...
    Private Sub MnuElimina_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ctxEliminar.Click
        ...
        If dgDatos.SelectedRows.Count > 0 Then
            ...
            If MsgBox("ESTA SEGURO DE ELIMINAR EL REGISTRO MARCADO", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Mensaje del Sistema") = MsgBoxResult.Yes Then
                If BuscaEscalar("select count(*) from suspensiones where grado=@grado", ListaParametros(lpara)) > 0 Then
                    MsgBox("REGISTRO POSEE REFERENCIA", MsgBoxStyle.Critical, "Mensaje del Sistema")
                    Exit Sub
                End If
                'verificar si tiene referencia en inscripciones
                cadena = "delete from gradoslesion where grado=@grado"
                EjecutarQuery(cadena, ListaParametros(lpara))
    ...
    Private Sub MnuModifica_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ctxModificar.Click
        ...
        If dgDatos.SelectedRows.Count > 0 Then
            accion = 1
            EnabilizarMenu(False)
            f = tabla.Rows(dgDatos.SelectedRows(0).Index)
