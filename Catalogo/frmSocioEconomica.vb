...
'*************************************************************************************************
'*************************************************************************************************
'**  FORMULARIO:    FRMSOCIOECONOMICA.VB MIEMBRO DE NOMINA.SLN                                  **
'**  AUTOR:         IVAN TRUJILLO                                                               **
'**  FECHA:         01/01/2008                                                                  **
'**  FECHA DE MOD:  --/--/----                                                                  **
'**  DESCRIPCION:                                                                               **
...
Public Class frmSocioEconomica
    ...
    Private Sub btnLimpiar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLimpiar.Click
        ...
        cadena = "SELECT tipositua, nombre " &
                 "FROM situaecono where empresa=@empresa order by tipositua"
        ...
        End If
        textnum.Text = BuscaEscalar("select coalesce( max(tipositua), 0) from situaecono where empresa=@empresa", ListaParametros(lpara)) + 1
        EnabilizarMenu(True)
    End Sub
    ...
    Private Sub btnAgregar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAgregar.Click
        ...
        End If
        lpara("empresa") = empresa
        lpara("tipositua") = textnum.Text
        lpara("nombre") = TextNombre.Text
        Select Case accion
            Case 0
                cadena = "insert situaecono (empresa,tipositua,nombre) values(@empresa,@tipositua,@nombre)"
            Case 1
                cadena = "update situaecono set nombre=@nombre where tipositua=@tipositua and empresa=@empresa"
        End Select
    ...
    Private Sub MnuElimina_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ctxEliminar.Click
        ...
        If dgDatos.SelectedRows.Count > 0 Then
            ...
            lpara("economica") = filaTemp.Item(0)
            lpara("empresa") = empresa
            If MsgBox("ESTA SEGURO DE ELIMINAR EL REGISTRO MARCADO", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Mensaje del Sistema") = MsgBoxResult.Yes Then
                If BuscaEscalar("select count(*) from emplegen where economica=@economica and empresa=@empresa", ListaParametros(lpara)) > 0 Then
                    MsgBox("REGISTRO POSEE REFERENCIA", MsgBoxStyle.Critical, "Mensaje del Sistema")
                    Exit Sub
                End If
                'verificar si tiene referencia en inscripciones
                cadena = "delete from situaecono where tipositua=@economica and empresa=@empresa"
                EjecutarQuery(cadena, ListaParametros(lpara))
