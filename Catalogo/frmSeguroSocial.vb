...
'*************************************************************************************************
'*************************************************************************************************
'**  FORMULARIO:    FRMSEGUROSOCIAL.VB MIEMBRO DE NOMINA.SLN                                    **
'**  AUTOR:         IVAN TRUJILLO                                                               **
'**  FECHA:         01/01/2008                                                                  **
'**  FECHA DE MOD:  --/--/----                                                                  **
'**  DESCRIPCION:                                                                               **
...
#Region "TRANSACCION"
    ...
    Private Sub btnCodigo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscaCodigo.Click
        ...
        cadena = "select transac, nombre from tipotran where empresa=@empresa and nombre like '%' + @nombre + '%' and tipomov='D'  order by transac"
    ...
    Private Sub ValidaCodigo()
        ...
        If valida_tipo_Entero(textCodigo.Text, 1) = True Then
            If BuscaEscalar("select count (*) from tipotran where empresa=@empresa and transac=@transac", ListaParametros(lpara)) = 0 Then
                ...
                Exit Sub
            End If
            cadena = "select transac, nombre from tipotran where empresa=@empresa and transac=@transac and tipomov='D'"
            abrir_conexion(cn)
...
#End Region
    ...
    Private Sub btnLimpiar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLimpiar.Click
        ...
        cadena = "SELECT t.tiposeguro, t.nombre, por, t.transac, ti.nombre " &
                 " FROM segurosocial t INNER JOIN  tipotran ti on t.empresa=ti.empresa and t.transac=ti.transac " &
                 " where t.empresa=@empresa order by tiposeguro"
        ...
        End If
        textnum.Text = BuscaEscalar("select coalesce( max(tiposeguro), 0) from segurosocial where empresa=@empresa", ListaParametros(lpara)) + 1
        EnabilizarMenu(True)
    End Sub
    ...
    Private Sub Vista(ByVal dgVista As DataGridView)
        With dgVista
            ...
            .Columns(4).Width = 238
            'AltoGridView(18, tabla, 292, 656, dgVista)
        End With
    End Sub
    ...
    Private Sub btnAgregar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAgregar.Click
        ...
        If validetError(textnum, ep1) = False Or validetError(TextNombre, ep1) = False Or
           validetError(textPorcentaje, ep1) = False Or validetError(textCodigo, ep1) = False Then
            Exit Sub
        End If
        lpara("empresa") = empresa
        lpara("tipoSeguro") = textnum.Text
        lpara("nombre") = TextNombre.Text
        lpara("por") = CDec(textPorcentaje.Text)
        lpara("transac") = textCodigo.Text
        Select Case accion
            Case 0
                cadena = "insert segurosocial (empresa,tiposeguro,nombre,por,transac) 
                          values(@empresa,@tipoSeguro,@nombre,@por,@transac)"
            Case 1
                cadena = "update segurosocial set nombre=@nombre, por=@por, transac=@transac  where tiposeguro=@tipoSeguro and empresa=@empresa"
        End Select
    ...
    Private Sub MnuElimina_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ctxEliminar.Click
        ...
        If dgDatos.SelectedRows.Count > 0 Then
            ...
            lpara("tipoSeguro") = filaTemp.Item(0)
            lpara("empresa") = empresa
            If MsgBox("ESTA SEGURO DE ELIMINAR EL REGISTRO MARCADO", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Mensaje del Sistema") = MsgBoxResult.Yes Then
                If BuscaEscalar("select count(*) from contratos1 where tiposeguro=@tipoSeguro and empresa=@empresa", ListaParametros(lpara)) > 0 Then
                    MsgBox("REGISTRO POSEE REFERENCIA", MsgBoxStyle.Critical, "Mensaje del Sistema")
                    Exit Sub
                End If
                'verificar si tiene referencia en inscripciones
                cadena = "delete from segurosocial where tiposeguro=@tipoSeguro and empresa=@empresa"
                EjecutarQuery(cadena, ListaParametros(lpara))
    ...
    Private Sub MnuModifica_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ctxModificar.Click
        ...
        If dgDatos.SelectedRows.Count > 0 Then
            ...
            textPorcentaje.Text = f.Item(2)
            textCodigo.Text = f.Item(3)
    ...
    Private Sub Foco(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles textnum.Enter, _
        TextNombre.Enter, textPorcentaje.Enter, textCodigo.Enter
    ...
    Private Sub textPorcentaje_Validated(ByVal sender As Object, ByVal e As System.EventArgs) Handles textPorcentaje.Validated
        ...
        If textPorcentaje.Text.Trim <> "" Then
            If CDec(textPorcentaje.Text) > 100 Or CDec(textPorcentaje.Text) = 0 Then
                MsgBox("VALOR DEL PORCENTAJE DEBE SER ENTRE 0 Y 100 ", MsgBoxStyle.Exclamation, "Mensaje del Sistema")
    ...
    Private Sub Deja(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles textnum.Leave, _
         TextNombre.Leave, textPorcentaje.Leave, textCodigo.Leave
        desactiva(sender)
    End Sub
