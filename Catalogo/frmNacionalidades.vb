...
'*************************************************************************************************
'*************************************************************************************************
'**  FORMULARIO:    FRMNACIONALIDADES.VB MIEMBRO DE NOMINA.SLN                                  **
'**  AUTOR:         IVAN TRUJILLO                                                               **
'**  FECHA:         01/01/2008                                                                  **
'**  FECHA DE MOD:  --/--/----                                                                  **
'**  DESCRIPCION:                                                                               **
...
Public Class frmNacionalidades
    ...
    Private Sub btnLimpiar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLimpiar.Click
        ...
        cadena = "SELECT nacional, nombre " &
                 "FROM nacionalidades order by nacional "
    ...
    Private Sub Vista(ByVal dgVista As DataGridView)
        With dgVista
            .Columns(0).HeaderText = "Nacionalidad"
            .Columns(0).Width = 150
    ...
    Private Sub btnAgregar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAgregar.Click
        ...
        If validetError(textnum, ep1) = False Or validetError(TextNombre, ep1) = False Then
            Exit Sub
        End If
        lpara("nacional") = textnum.Text.Trim
        lpara("nombre") = TextNombre.Text
        Select Case accion
            Case 0
                cadena = "select count(*) from nacionalidades where nacional=@nacional"
                If BuscaEscalar(cadena, ListaParametros(lpara)) > 0 Then
                    MsgBox("CODIGO DE NACIONALIDAD YA EXISTE, VERIFIQUE", MsgBoxStyle.Exclamation, "Mensaje del Sistema")
                    textnum.Focus()
                    Exit Sub
                End If
                cadena = "insert nacionalidades(nacional,nombre) values(@nacional,@nombre)"
            Case 1
                cadena = "update nacionalidades set nombre=@nombre where nacional=@nacional"
        End Select
    ...
    Private Sub MnuElimina_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ctxEliminar.Click
        ...
        If dgDatos.SelectedRows.Count > 0 Then
            ...
            lpara("nacional") = filaTemp.Item(0)
            If MsgBox("ESTA SEGURO DE ELIMINAR EL REGISTRO MARCADO", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Mensaje del Sistema") = MsgBoxResult.Yes Then
                If BuscaEscalar("select count(*) from emplegen where nacional=@nacional", ListaParametros(lpara)) > 0 Then
                    MsgBox("REGISTRO POSEE REFERENCIA", MsgBoxStyle.Critical, "Mensaje del Sistema")
                    Exit Sub
                End If
                'verificar si tiene referencia en inscripciones
                cadena = "delete from nacionalidades where nacional=@nacional"
                EjecutarQuery(cadena, ListaParametros(lpara))
